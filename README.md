...
- **`.github/workflows/collect.yml`** runs the collector hourly, commits the refreshed JSON, and deploys `site/` to GitHub Pages. GitHub's cron is best-effort and often skips slots, so it is scheduled three times an hour.
- **`scripts/install-local-trigger.sh`** installs a launchd agent on a Mac that dispatches the workflow hourly as a backstop (needs `gh auth login`).
